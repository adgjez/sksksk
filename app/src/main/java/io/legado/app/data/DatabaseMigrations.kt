...
object DatabaseMigrations {
    ...
    val migrations: Array<Migration> by lazy {
        arrayOf(
            ...
            migration_99_100,
            // AI features port from Rimchars/legado feature/discovery-suite-select.
            // Combines source migrations 100..109 into a single step.
            migration_100_109
        )
    }
    ...
    /**
     * AI features port from Rimchars/legado `feature/discovery-suite-select` branch.
     *
     * Combines source migrations 100..109 into a single step. Original migrations
     * reference `book_characters` and `read_aloud_bgm_*` tables that this base
     * does not have, so the port only ships the AI-related tables (Agent, Memory,
     * World Book, Image, Read Aloud Cache, Chapter Summary, FTS).
     *
     * If you later port those missing tables, replace this single migration with
     * the original per-step migration chain.
     */
    private val migration_100_109 = object : Migration(100, 109) {
        override fun migrate(db: SupportSQLiteDatabase) {
            // --- ai_agent_sessions / ai_agent_jobs / ai_agent_traces ---
            db.execSQL(
                """
            ...
            // --- ai_memory_items / ai_memory_fragments + FTS ---
            db.execSQL(
                """
            ...
            // FTS4 virtual tables (must come AFTER the source tables for population)
            db.execSQL(
                """
            ...
            db.execSQL(
                """
                INSERT OR IGNORE INTO `ai_memory_items_fts` (`memoryId`, `subject`, `predicate`, `objectValue`, `content`)
                SELECT `memoryId`, `subject`, `predicate`, `objectValue`, `content` FROM `ai_memory_items`
                """.trimIndent()
            )
            db.execSQL(
                """
                INSERT OR IGNORE INTO `ai_memory_fragments_fts` (`fragmentId`, `title`, `content`, `chapterTitle`)
                SELECT `fragmentId`, `title`, `content`, `chapterTitle` FROM `ai_memory_fragments`
                """.trimIndent()
            )

            // --- ai_generated_images / ai_image_groups ---
            db.execSQL(
                """
            ...
            // --- Companion features required by AI code (port from source) ---

            // book_characters + book_character_relations
            db.execSQL(
                """
                CREATE TABLE IF NOT EXISTS `book_characters` (
                    ...
                    `firstChapterIndex` INTEGER NOT NULL DEFAULT -1,
                    `lastChapterIndex` INTEGER NOT NULL DEFAULT -1,
            ...
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_book_characters_bookKey` ON `book_characters` (`bookKey`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_book_characters_name` ON `book_characters` (`name`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_book_characters_bookKey_name` ON `book_characters` (`bookKey`, `name`)")
            db.execSQL(
                """
                CREATE TABLE IF NOT EXISTS `book_character_relations` (
                    `id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
                    `bookKey` TEXT NOT NULL DEFAULT '',
                    `sourceCharacterId` INTEGER NOT NULL DEFAULT 0,
                    `sourceName` TEXT NOT NULL DEFAULT '',
                    `targetCharacterId` INTEGER NOT NULL DEFAULT 0,
            ...
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_book_character_relations_bookKey` ON `book_character_relations` (`bookKey`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_book_character_relations_sourceCharacterId` ON `book_character_relations` (`sourceCharacterId`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_book_character_relations_targetCharacterId` ON `book_character_relations` (`targetCharacterId`)")
            ...
            // read_aloud_speaker_groups + items
            db.execSQL(
                """
